...
public class PrometeoCarController : NetworkBehaviour
{
    ...
    */
    ...
    public SyncEvent<bool> OnDriftStateChanged = new SyncEvent<bool>();

    #region PredictedData
    ...
    protected override void OnSpawned()
    {
        base.OnSpawned();
        _isEnabled = isOwner;
        OnDriftStateChanged.AddListener(HandleDriftVisual);
    ...
    public void HandleDriftVisual(bool state)
    {
        if (!state)
        {
    ...
    void Start()
    {
        ...
        if (!useEffects)
        {
            ...
            if (isServer)
                OnDriftStateChanged?.Invoke(false);
            else
            {
                OnDriftStateChangedServerRPC(false);
            }
        }
        ...
        if (useEffects)
        {
            try
            {
                if (isDrifting)
                {
                    ...
                    if (isServer)
                        OnDriftStateChanged?.Invoke(true);
                    else
                    {
                        OnDriftStateChangedServerRPC(true);
                    }
                }
                else if (!isDrifting)
                {
    ...
    [ServerRpc]
    private void OnDriftStateChangedServerRPC(bool state)
    {
        OnDriftStateChanged?.Invoke(state);
    }
}
